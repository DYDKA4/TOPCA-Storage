...
def start_attribute_definition(father_node_vid, varargs):
    if len(varargs) < 2:
        abort(400)
...
def update_attribute_definition(service_template_vid, father_node_vid, value, value_name, varargs: list, type_update,
                                cluster_name):
    property_vid_to_update = start_attribute_definition(father_node_vid, varargs)
    if len(varargs) == 2:
...
def get_attribute_definition(father_node_vid, value, value_name, varargs: list):
    attribute_vid_to_update = start_attribute_definition(father_node_vid, varargs)
    if len(varargs) == 2:
        vertex_value = fetch_vertex(attribute_vid_to_update, 'AttributeDefinition')
        vertex_value = vertex_value.as_map()
        if value_name == 'type':
            return form_result(attribute_vid_to_update, value_name)
        elif value_name in vertex_value.keys():
            if value == vertex_value.get(value_name).as_string():
                return attribute_vid_to_update.as_string()
        else:
            abort(501)
    elif varargs[2] == 'constraints':
        destination = find_destination(attribute_vid_to_update, value_name)
