...
def start_group_type(father_node_vid, varargs):
    if len(varargs) < 2:
        abort(400)
...
def update_group_type(father_node_vid, value, value_name, varargs: list, type_update, cluster_name):
    group_type_vid_to_update = start_group_type(father_node_vid, varargs)
    if len(varargs) == 2:
...
def get_group_type(father_node_vid, value, value_name, varargs: list):
    group_vid_to_update = start_group_type(father_node_vid, varargs)
    if len(varargs) == 2:
        vertex_value = fetch_vertex(group_vid_to_update, 'GroupType')
        vertex_value = vertex_value.as_map()
        if value_name == 'derived_from':
