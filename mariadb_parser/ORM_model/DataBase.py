...
class ArtifactStorage(Base):
    ...
    def __repr__(self):
        return "<artifact_storage(name='%s', data='%s')>" % (self.name, self.data)
...
class DBCapability(Base):
    ...
    def __repr__(self):
        return "<capability(name='%s', data='%s')>" % (self.name, self.value)
...
class DBCapabilityAttributeAndProperty(Base):
    ...
    value_storage_id = Column(
        String(length=36),
        ForeignKey("value_storage.id", ondelete="CASCADE"),
        nullable=False,
    )
...
class InstanceModelInputAndOutput(Base):
    ...
    id = Column(String(length=36), primary_key=True, nullable=False)
    instance_model_id = Column(
        String(length=36),
        ForeignKey("instance_model.id", ondelete="CASCADE"),
        nullable=False,
    )
...
class DBNodeInterfaceOperation(Base):
    ...
    id = Column(String(length=36), primary_key=True, nullable=False)
    node_interface_id = Column(
        String(length=36),
        ForeignKey("node_interface.id", ondelete="CASCADE"),
        nullable=False,
    )
    name = Column(String(length=255), nullable=False)
...
class DBRelationshipInterfaceOperation(Base):
    ...
    id = Column(String(length=36), primary_key=True, nullable=False)
    relationship_interface_id = Column(
        String(length=36),
        ForeignKey("relationship_interface.id", ondelete="CASCADE"),
        nullable=False,
    )
    name = Column(String(length=255), nullable=False)
    implementation = Column(String(length=255))
...
class TypeStorageToArtifactStorage(Base):
    ...
    artifact_storage_id = Column(
        String(length=36),
        ForeignKey("artifact_storage.id"),
        primary_key=True,
        nullable=False,
    )
    type_storage_id = Column(
        String(length=36), ForeignKey("type.id"), primary_key=True, nullable=False
    )
