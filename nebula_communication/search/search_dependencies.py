...
def find_dependencies_for_vertex(vid, vertex_type_system):
    result = {
        'ArtifactType': set(),
        'CapabilityType': set(),
        'DataType': set(),
    ...
    while result_change:
        print(steps)
        # artifact_type = find_all_edges(vid, steps, vertex_type_system='ArtifactType')
        all_vid = find_all_edges(vid, steps)
        for vid_total in list(zip(all_vid.column_values('id'), all_vid.column_values('props'))):
            result['Total_amount_of_vertex'].add((vid_total[0].as_string(),
...
#
# res = find_dependencies_for_vertex('"90f0bc82-41bd-43b2-83ad-16deb55252d5"', 'NodeType')
